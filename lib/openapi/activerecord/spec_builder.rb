module Openapi::ActiveRecord
end

module Openapi::ActiveRecord::SpecBuilder
  ...
  class_methods do
    ...
    def build_openapi_paths
      ...
      warn_on_undocumented_actions(routes) if Rails.env.development?
    end
        ...
        swagger_path path do
          if include_index
            operation :get do
              ...
              scopes.each do |_k, config|
                scope_name = config[:as]
                scope_type = config[:type]

                scope_type = :string if scope_type == :default
                ...
                parameter do
                  ...
                  key :format, :int32 if scope_type == :integer
                end
              end
    ...
    def warn_on_undocumented_actions(routes)
      ...
      return if no_spec_methods.empty?
    ...
    def action_specification_exists?(method, path)
      swagger_nodes = send(:_swagger_nodes)
      node_map = swagger_nodes[:path_node_map]

      node_map.key?(path) && node_map[path].data.key?(method)
    end
  end
end
